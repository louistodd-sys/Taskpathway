...
function stepToForm(s: Step): StepFormData {
  return {
    ...
    video_captions: s.video_captions ?? [],
  }
}
...
export default function TaskEditor({ companyId, userId, userRole, task, steps: initialSteps }: TaskEditorProps) {
  ...
  const handleSave = useCallback(async (options: {
    ...
    if (isEdit) {
      const { error } = await supabase.from('documents').update(documentData).eq('id', task!.id)
      if (error) { setError(error.message); setSaving(false); return }
    } else {
      const { data, error } = await supabase.from('documents')
        .insert({ ...documentData, current_version_number: '1' }).select().single()
      if (error) { setError(error.message); setSaving(false); return }
      documentId = data.id
    }
    ...
    if (toDelete.length) await supabase.from('steps').delete().in('id', toDelete)
    ...
    await recordChange(supabase, { companyId, userId, documentId: documentId!, documentTitle: form.title.trim(), actionType, summary })

    setSaving(false); setSaved(true)
    setTimeout(() => setSaved(false), 2000)
    if (!isEdit && documentId) router.push(`/edit/${documentId}`)
    else router.refresh()
  }, [form, steps, companyId, userId, task, isEdit, initialSteps, supabase, router])

  const handleSubmitForReview = () => handleSave({ statusOverride: 'In Review', submittedAt: new Date().toISOString() })
  const handleApprove = () => handleSave({ statusOverride: 'Approved', reviewedAt: new Date().toISOString(), reviewerNote: '' })
  const handleReject = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <button onClick={() => router.push('/library')} className="p-2 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{isEdit ? 'Edit Task' : 'Create Task'}</h1>
          {task && (
            <div className="flex items-center gap-2 mt-1">
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusBadgeColors[task.status] ?? 'bg-gray-100 text-gray-600'}`}>{task.status}</span>
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {canApprove && task?.status === 'In Review' && (
            <>
              <button onClick={() => { setShowRejectInput(v => !v); setError('') }} disabled={saving}
          ...
          {canApprove && (task?.status === 'Draft' || task?.status === 'Rejected' || !isEdit) && (
            <button onClick={() => handleSave({ statusOverride: 'Approved', reviewedAt: new Date().toISOString() })} disabled={saving}
      ...
      {showRejectInput && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl">
          ...
          <div className="flex gap-2 mt-2">
            <button onClick={handleReject} disabled={saving} className="px-3 py-1.5 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 disabled:opacity-50">Confirm rejection</button>
            <button onClick={() => { setShowRejectInput(false); setRejectNote('') }} className="px-3 py-1.5 border border-gray-300 text-gray-600 rounded-lg text-sm hover:bg-gray-50">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className={`bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-5 ${isReadOnly ? 'opacity-75 pointer-events-none' : ''}`}>
        ...
        <div>
          ...
          <input value={form.title} onChange={e => setField('title', e.target.value)} placeholder="e.g. Machine start-up procedure"
            className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Department (optional)</label>
          <input value={form.department} onChange={e => setField('department', e.target.value)} placeholder="e.g. Production, Maintenance, Quality"
            className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Type</label>
            <select value={form.document_type} onChange={e => setField('document_type', e.target.value as 'Task' | 'Work Instruction')}
          ...
          <div>
            ...
            <input value={form.estimated_time} onChange={e => setField('estimated_time', e.target.value)} placeholder="e.g. 15 mins"
              className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Critical safety information (optional)</label>
          <textarea value={form.critical_safety_info} onChange={e => setField('critical_safety_info', e.target.value)} rows={2}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {form.required_tools.map(tool => (
              <span key={tool} className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 text-xs font-medium px-2.5 py-1 rounded-full">
                {tool}
                <button type="button" onClick={() => removeTool(tool)} className="hover:text-indigo-900"><X className="w-3 h-3" /></button>
              </span>
            ))}
          </div>
          <div className="flex gap-2">
            ...
            <button type="button" onClick={addTool} className="px-3 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm hover:bg-gray-200 transition-colors">
